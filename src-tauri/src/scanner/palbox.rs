...
pub fn scan_box_parallel(
    ...
    // Drain captured one slot at a time so images are freed immediately
    // after each slot is processed — keeps peak memory to ~1 slot's worth.
    let phase2_start = Instant::now();
    let captured_count = captured.len();

    let mut all_results: Vec<(SlotResult, String)> = Vec::with_capacity(captured_count);
    for cs in captured.drain(..) {
        if SCAN_ABORT.load(Ordering::Relaxed) {
    ...
    let mut results: Vec<SlotResult> = Vec::with_capacity(all_results.len());
    let mut log: Vec<String> = Vec::with_capacity(all_results.len());
    for (r, l) in all_results {
    ...
    log.push(format!(
        "Phase 1 capture: {phase1_ms}ms ({captured_count} slots)",
    ));
